...
- **Stato**: da decidere. Proposta: task Jira nell'Epic EA-3 per confermare (o correggere) questa scelta di fallback, valutando anche se serva un backfill del campo `type` sui documenti `checklists/{id}` esistenti in produzione creati prima di EA-123, riferimento F-7.
