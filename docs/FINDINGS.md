...
- **Stato**: risolto in questa PR. Aggiunto `type` a `TemplateItemForm`/`EMPTY_ITEM`, un selettore Dropdown per riga item, e corretta la ricostruzione del payload per includere `type`. Non copre l'editing del `type` di un item di template già esistente in una vista dedicata (resta implicito nella stessa dialog di modifica) né la resa condizionale delle colonne, che restano scope di EA-128.
