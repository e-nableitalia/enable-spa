...
## F-7: Il `type` selezionato nella form di aggiunta nuovo item di `ChecklistPanel.tsx` non viene persistito da `addChecklistItem`

- **Descrizione**: EA-128 introduce nella form di aggiunta item di `ChecklistPanel.tsx` un selettore `Tipo` (`boolean`/`generic`/`numeric`) coerente col discriminante introdotto da EA-123, e il payload inviato a `addDeviceRequestChecklistItem` include ora `type`. Tuttavia la funzione core `addChecklistItem.ts` — a differenza di `createChecklist.ts` — non legge né valida ancora questo campo (non è nella destructuring di `request.data`): lo ignora silenziosamente. Di conseguenza ogni item aggiunto tramite questa form nasce senza `type` persistito e, coerentemente col comportamento "fail closed" introdotto da questa Story (colonna Quantità nascosta quando `type` è assente), la colonna Quantità resterà nascosta per quell'item dopo il prossimo reload, indipendentemente da cosa l'utente aveva selezionato in fase di aggiunta.
- **Evidenza**:
  - `functions/backend/src/organizer/addChecklistItem.ts:34-40` — destructuring di `request.data` privo di `type`
  - `enable-device/src/components/checklist/ChecklistPanel.tsx` — payload di `addDeviceRequestChecklistItem` include `type: newItem.type ?? undefined`, ma nessun consumer backend lo persiste ancora
- **Story/PR/sessione di provenienza**: rilevato durante l'implementazione della Story Jira EA-128, sessione 2026-08-03. Comportamento già anticipato dal commento in `checklistItemStatus.ts` ("`type` ... prerequisito strutturale per il discriminante che verrà introdotto sulle funzioni CRUD nelle Story successive di questa Epic"): non è una regressione introdotta da EA-128, ma un gap che questa Story rende visibile lato UI.
- **Stato**: da decidere. Proposta: Task Jira nell'Epic del modello item di checklist per estendere `addChecklistItem.ts` (e il wrapper `addDeviceRequestChecklistItem`) ad accettare/validare `type` tramite `isChecklistItemType`, coerentemente con EA-123, cosicché gli item aggiunti dopo la creazione della checklist possano avere la colonna Quantità visibile quando pertinente.
