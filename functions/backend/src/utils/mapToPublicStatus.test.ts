...
describe("mapToPublicStatus", () => {
  // Scenario: stato 'inviata' mappato al gruppo 'da validare'
  ...
  // Scenario: il gruppo 'fabbricazione in corso' resta invariato (non regressione)
  it.each([
    ...
    "spedita",
    "followup famiglia"
  ])("keeps '%s' mapped to 'fabbricazione in corso' (regression)", (status) => {
